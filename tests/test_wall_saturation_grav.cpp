...
class WallSaturationGrav : public testing::TestWithParam<torch::DeviceType> {};

TEST_P(WallSaturationGrav, phase_change_preserves_energy_and_water) {
  ...
  for (bool condense : {true, false}) {
    ...
    for (bool upper : {false, true}) {
      ...
      auto b = MeshBlock(
          MeshBlockOptionsImpl::from_yaml("test_wall_saturation_grav.yaml"));
      ...
      auto z = b->pcoord->x1v;
      constexpr double grav1 = 9.8;
      auto p = 1.e5 * torch::exp(-grav1 * z / (rgas * temp));
      w[IPR].copy_(p.view({1, 1, -1}));
      w[IDN].copy_((p / (rgas * temp)).view({1, 1, -1}));
      if (upper)
        z = 64. - z;
      if (condense) {
      ...
      // With gravity, hydro_u[IPR] is not a closed energy; water mass is.
      EXPECT_LT(dw, 1.e-12);
    }
  }
}
