...
// fvm
#include <fvm/implicit/alloc_eigen.h>
#include <fvm/implicit/tridiag_thomas.h>
...
TEST(TridiagThomas, forward_sweep) {
  ...
  c10::ScalarType dtype = c10::kDouble;
  alloc_eigen_cuda<5>(a, b, c, delta, corr, ncol, nlayer, dtype);
  ...
  free_eigen_cuda(a, b, c, delta, corr);
  cudaFree(du);
  cudaFree(w);
  delete[] aa;
}
