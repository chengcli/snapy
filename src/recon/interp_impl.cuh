...
template <typename T>
__device__ T interp_point_poly3_impl(T const *w, int v, int start,
                                     int stride_var, int stride_dim,
                                     bool right) {
  constexpr T cm[3] = {-1. / 3., 5. / 6., -1. / 6.};
  T out = 0.;
  for (int k = 0; k < 3; ++k) {
    out += (right ? cm[2 - k] : cm[k]) *
...
template <typename T>
__device__ T interp_point_poly5_impl(T const *w, int v, int start,
                                     int stride_var, int stride_dim,
                                     bool right) {
  constexpr T cm[5] = {-1. / 20., 9. / 20., 47. / 60., -13. / 60., 1. / 30.};
  T out = 0.;
  for (int k = 0; k < 5; ++k) {
    out += (right ? cm[4 - k] : cm[k]) *
           w[v * stride_var + (start + k) * stride_dim];
  }
  return out;
}
...
template <typename T>
__device__ T interp_point_weno3_impl(T const *w, int v, int start,
                                     int stride_var, int stride_dim,
                                     bool right) {
...
template <typename T>
__device__ T interp_point_weno5_impl(T const *w, int v, int start,
                                     int stride_var, int stride_dim,
                                     bool right) {
  constexpr T cm[9][5] = {
      {-1. / 6., 5. / 6., 1. / 3., 0., 0.},
