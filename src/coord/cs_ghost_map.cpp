...
struct CSGhostMap {
  ...
  int side_s;  /* source side id (SIDE_L/R/B/T) */
  int j_along; /* along-edge index on target (0..N-1) */
  int depth;   /* ghost depth (1..nghost) */
 ...
 */
static inline void cs_build_ghost_map_table(int N, int nghost,
                                            int apply_rev_flag,
                                            CSGhostMap *gmap) {
