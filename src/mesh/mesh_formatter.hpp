...
struct fmt::formatter<snap::MeshOptions> {
  ...
  auto format(const snap::MeshOptions& p, FormatContext& ctx) const {
...
struct fmt::formatter<snap::LogicalLocation> {
  ...
  // Define the format function for MeshOptions
  template <typename FormatContext>
  auto format(const snap::LogicalLocation& p, FormatContext& ctx) const {
...
struct fmt::formatter<snap::OctTreeOptions> {
  ...
  // Define the format function for MeshOptions
  template <typename FormatContext>
  auto format(const snap::OctTreeOptions& p, FormatContext& ctx) const {
    return fmt::format_to(ctx.out(),
