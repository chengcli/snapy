...
// (273, 373)
inline torch::Tensor svp_h2o_Antoine(torch::Tensor T) {
  auto result1 = torch::pow(10., 5.40221 - (1838.675 / (T - 31.737)));
  auto result2 = torch::pow(10., 5.20389 - (1733.926 / (T - 39.485)));
  auto result3 = torch::pow(10., 5.0768 - (1659.793 / (T - 45.854)));
  auto result4 = torch::pow(10., 5.08354 - (1663.125 / (T - 45.622)));
  auto result =
      torch::where(T < 303., result1,
                   torch::where(T < 333., result2,
                                torch::where(T < 363., result3, result4)));
