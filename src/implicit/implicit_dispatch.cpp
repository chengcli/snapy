...
#include "vic_solve_impl.h"

namespace snap {

template <int N>
void vic_solve_cpu(at::TensorIterator &iter, double dt, double grav, int il,
                   int iu, int dir) {
    ...
    iter.for_each(
        [&](char **data, const int64_t *strides, int64_t n) {
          for (int i = 0; i < n; i++) {
            ...
            auto a = reinterpret_cast<Eigen::Matrix<scalar_t, N, N> *>(
                data[5] + i * strides[5]);
            auto b = reinterpret_cast<Eigen::Matrix<scalar_t, N, N> *>(
                data[6] + i * strides[6]);
            auto c = reinterpret_cast<Eigen::Matrix<scalar_t, N, N> *>(
                data[7] + i * strides[7]);
