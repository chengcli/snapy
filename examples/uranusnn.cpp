...
using namespace std;
static Real Rd, cp, T0, Rp, grav, eq_heat_flux, sday, syear, dday, Omega, sigma,
    emissivity, adtdz, cv, spinupflux, flux_ratio, Tint, initheatdecay, Kt,
    sponge_tau, spongeheight, conv_time, xHe, tempmean, tempstd, heatthr,
    heatsf, fluxmean, fluxstd, umumean, umustd;
...
torch::Tensor torch_normalize_standard(torch::Tensor x, double mean,
                                       double std) {
  return (x - mean) / std;
}
...
torch::Tensor regrid(const std::vector<double> &x, const std::vector<double> &y,
  ...
  return torch_normalize_standard(
      torch::tensor(result, torch::TensorOptions().dtype(torch::kFloat32))
          .view({1, 100, 1}),
      tempmean, tempstd);
}

std::vector<double> degrid(const std::vector<double> &x,
                           const torch::Tensor normy,
                           const std::vector<double> &xq) {
  std::vector<double> result(xq.size());
...
void Forcing(MeshBlock *pmb, Real const time, Real const dt,
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)             // last index is vertical
    for (int j = pmb->js; j <= pmb->je; ++j) {
      ...
      torch::Tensor regridtemp =
          regrid(pressvec, tempvec, basepress);  // normalize and regrid
      Real lat, lon;
      pexo3->GetLatLon(&lat, &lon, k, j, pmb->is);
      torch::Tensor global_features =
          calcglobal(lat, time);  // normalized global features
      ...
      heating = degrid(basepress, output,
                       pressvec);  // pressure decreases, heigh increases with
                                   // increasing index
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        du(IEN, k, j, i) +=
            dt * (cp - Rd) * w(IDN, k, j, i) * heating[i - pmb->is];
      }
    }
}
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  // int is = pmb->is, ie = pmb->ie, js = pmb->js, je = pmb->je, ks = pmb->ks,
  // ke = pmb->ke; Real currentforcing[ke-ks][je-js][ie-is] = {};
  tempmean =
      pin->GetReal("problem", "tempmean");  // specific cooling/heating rate
  tempstd = pin->GetReal("problem", "tempstd");  // specific cooling/heating
                                                 // rate
  heatthr = pin->GetReal("problem", "heatthr");  // specific cooling/heating
                                                 // rate
  heatsf = pin->GetReal("problem", "heatsf");  // specific cooling/heating rate
  umumean = pin->GetReal("problem", "umumean");  // specific cooling/heating
                                                 // rate
  umustd = pin->GetReal("problem", "umustd");  // specific cooling/heating rate
  fluxmean =
      pin->GetReal("problem", "fluxmean");  // specific cooling/heating rate
  fluxstd = pin->GetReal("problem", "fluxstd");  // specific cooling/heating
                                                 // rate
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  */
  ...
  const size_t ncell =
      static_cast<size_t>(26) * 96 *
      144;  // total cells in mesh, less than total cells in data
  ...
  if (Globals::my_rank == 0) {
    std::cout << "IC check rho[0]=" << rho[0] << " vel1[0]=" << vel1[0]
              << std::endl;
  }
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin) {
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)             // last index is vertical
    for (int j = pmb->js; j <= pmb->je; ++j) {
      ...
      pexo3->GetLatLon(&lat, &lon, k, j, pmb->is);
      torch::Tensor global_features =
          calcglobal(lat, pmb->pmy_mesh->time);  // normalized global features
      std::vector<torch::jit::IValue> inputs;
      inputs.push_back(regridtemp);       // required
      inputs.push_back(global_features);  // optional global_features
      inputs.push_back(c10::nullopt);     // optional sequence_mask = None
