...
# op_coord.nc1(nx1 + 2 * nghost).nc2(nx2 + 2 * nghost).nc3(nx3 + 2 * nghost)
op_coord.x1min(0.0).x1max(2.0).x2min(-0.5).x2max(0.5).x3min(-0.5).x3max(0.5)
...
op_ib = InternalBoundaryOptions().solid_density(1.0e3).solid_pressure(1.0e9)
...
interior = block.part((0, 0, 0), extend_x1=1)
...
# internal boundary
solid = torch.zeros_like(w[0], dtype=torch.bool)
r1 = torch.sqrt((x1v - 0.5) ** 2 + (x2v) ** 2 + (x3v) ** 2)
solid[r1 < 2 * a] = 1

r2 = torch.sqrt((x1v - 0.6) ** 2 + (x2v - 0.25) ** 2 + (x3v) ** 2)
solid[r2 < 2 * a] = 1
